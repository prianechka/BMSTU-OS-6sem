...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Prianishnikov Alexander");
...
ssize_t fortune_write(struct file *filp, const char __user *buf, size_t length, loff_t *offp)
{
    if (length > PAGE_SIZE - cookie_idx + 1)
    {
        printk(KERN_ERR"+fortune: cookie_pot overflow error\n");
        return -ENOSPC; // Буфер для записи недоступен
    }
